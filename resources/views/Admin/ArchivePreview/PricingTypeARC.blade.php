    ...
    <div class="Table-1" >
        ...
        <div class="Table-con">
            ...
            <div class="Cnt2h">  
                ...
                <div class = "A-DD">
                       <a  href="{{route('PricingType.index')}}" class ="Add-P"> <i class="fa-solid fa-reply"></i> <span> Return </span></a> <!--Modal Button any -->
                </div>  
            ...
            @foreach($pricingtype as $pt)
              <tr>
                ...
                <td class="BTNS-EDTY">    
                    ...
                    <form id ="deleteP-form-{{$pt->id}}" action="{{Route('prtdestroy', $pt->id)}}" method = "POST"> 
                        @csrf
                        @method('DELETE')          
                     </form>

                     <button class="EditsT" onclick="resetData({{$pt->id}})" type="submit"><i class="fa-solid fa-repeat"></i> <span> RESTORE </span> </button> 
                     <form id ="reset-form-{{$pt->id}}" action="{{Route('prtstore', $pt->id)}}" method = "POST"> 
                        @csrf
                        @method('PUT')              
                      </form>
                </td>
